...
def plot_loss(train_data, val_data, test_data):
...
def plot_acc(train_data, val_data, test_data):
    _ = plt.figure(figsize=(12, 8))
...
class DATA:
    ...
    def test_data(self):
        assert self.x_train.shape == (3, 2700, 150), self.x_train.shape
        assert self.y_train.shape == (OUTPUT_SIZE, 2700), self.y_train.shape
...
def plots(model):
    ...
    pp = plot_loss(model.epoch_train_loss, model.epoch_val_loss, model.epoch_test_loss)
    pp.savefig(dir_name + "losses")

    pp = plot_acc(model.epoch_train_acc, model.epoch_val_acc, model.epoch_test_acc)
...
learning_rate = 0.05
batch_size = 30
rnn7 = RNN("rnn7", N, FEATURE_SIZE, OUTPUT_SIZE, NUM_TIME_STEPS)
rnn7.fit(data, lr=learning_rate, batch_size=batch_size, epochs=epochs, early_stopping=early_stopping)
plots(rnn7)
del rnn7
gc.collect()
...
learning_rate = 0.1
batch_size = 30
rnn8 = RNN("rnn8", N, FEATURE_SIZE, OUTPUT_SIZE, NUM_TIME_STEPS)
rnn8.fit(data, lr=learning_rate, batch_size=batch_size, epochs=epochs, early_stopping=early_stopping)
plots(rnn8)
del rnn8
gc.collect()
...
with open('data-section-c.json', 'w') as json_file:
...
batch_size = 30
rnn8 = RNN("rnn8", N, FEATURE_SIZE, OUTPUT_SIZE, NUM_TIME_STEPS)
rnn8.load_best_params("mini_p2_results/res_rnn8/")
y_preds, _ = rnn8.prediction(data.x_test, data.y_test)
cf = confusion_matrix(y_preds, data.y_test)
print("Confusion Matrix for rnn8:\n", cf)
